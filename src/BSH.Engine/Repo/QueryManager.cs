...
public class QueryManager : IQueryManager
{
    ...
    public async Task<VersionDetails> GetLastBackupAsync()
    {
        ...
        using (var reader = await dbClient.ExecuteDataReaderAsync(CommandType.Text, "SELECT * FROM versiontable WHERE versionStatus = 0 ORDER BY versionID DESC LIMIT 1", null))
        {
            ...
            {
                result = VersionDetailsMapper.FromReader(reader);
            }
    ...
    public List<VersionDetails> GetVersions(bool desc = true)
    {
            ...
            , null))
        {
            while (reader.Read())
            {
                result.Add(VersionDetailsMapper.FromReaderDetailed(reader));
            }
    ...
    public async Task<List<FileTableRow>> GetVersionsByFileAsync(string fileName, string filePath)
    {
        ...
        using (var dbClient = dbClientFactory.CreateDbClient())
        {
            ...
            while (await reader.ReadAsync())
            {
                result.Add(FileTableRowMapper.FromReaderFileVersion(reader));
            }
    ...
    public async Task<List<FileTableRow>> GetFilesByVersionAsync(string version, string path)
    {
        ...
        using (var dbClient = dbClientFactory.CreateDbClient())
        {
            ...
            while (await reader.ReadAsync())
            {
                result.Add(FileTableRowMapper.FromReaderFile(reader));
            }
    ...
    private async Task<List<VersionDetails>> GetAvailableVersionsForFileAsync(string fileName, string filePath)
    {
        ...
        using (var dbClient = dbClientFactory.CreateDbClient())
        {
            ...
            while (await reader.ReadAsync())
            {
                result.Add(VersionDetailsMapper.FromReader(reader));
            }
    ...
    private (string result, bool temp) ResolveFileFromStorage(System.Data.Common.DbDataReader reader, IStorageProvider storage, string password)
    {
        ...
        if (fileType == 1)
        {
            return (GetFileNameFromDrive(FileTableRowMapper.FromReaderFileVersion(reader)), false);
        }
