    ...
    public partial class frmBrowser : IStatusReport
    {
        ...
        private async void cmdTakeMeBack_Click(object sender, EventArgs e)
        {
            if (bSearch)
            {
                ...
                string Result = await BackupLogic.QueryManager.GetBackVersionWhereFileAsync(selectedVersion.Id, txtSearch.Text);
                if (Result is null)
                {
                    return;
                }

                selectedVersion = await BackupLogic.QueryManager.GetVersionByIdAsync(Result);
            ...
            {
                string Result = await BackupLogic.QueryManager.GetBackVersionWhereFilesInFolderAsync(selectedVersion.Id, selectedFolder);
                if (Result is null)
                {
                    return;
                }
        ...
        private async void cmdTakeMeLater_Click(object sender, EventArgs e)
        {
            if (bSearch)
            {
                ...
                // Version suchen, in der die Datei ist
                string Result = await BackupLogic.QueryManager.GetNextVersionWhereFileAsync(selectedVersion.Id, txtSearch.Text);
                if (Result is null)
                {
                    return;
                }
            ...
            {
                string Result = await BackupLogic.QueryManager.GetNextVersionWhereFilesInFolderAsync(selectedVersion.Id, selectedFolder);
                if (Result is null)
                {
                    return;
                }
